...
public class XmppChat
{
    ...
    public static void getPresenceResponse(XmppTalk xmppTalk)
    {
        ...
        String formatString = "<presence from='channel.%d@conference.%s/nfsw.%d' to='nfsw.%d@%s/EA-Chat' xml:lang='en'>"
                + "<x xmlns='http://jabber.org/protocol/muc#user'><item affiliation='none' role='none'/></x></presence>";
        xmppTalk.write(String.format(formatString, channelNumber, xmppIp, personaId, personaId, xmppIp));
                                     ...
                                     Integer channelNumber)
    {
        ...
        String newMessageBlock = String.format(
                "message from='nfsw.%s__%d@conference.%s/nfsw.%d' to='nfsw.%d@%s'", channelName, channelNumber,
    ...
    public static String getSystemMessage(Long targetPersonaId, String announcementText)
    {
        ...
        String announcementBlock = String
                .format("<message from='nfsw.engine.engine@conference.%s' id='JN_2578' to='nfsw.%d@%s'>"
    ...
    public String getWhisperMessage(Long targetPersonaId)
    {
        ...
        if (match.find()) {
            ...
            Long hostPersonaId = HttpState.getPersonaId();
            String xmppIp = Session.getXmppIp();

            String newMessageBlock = String.format("message from='nfsw.%d@%s' to='nfsw.%d@%s' type='%s'>",
